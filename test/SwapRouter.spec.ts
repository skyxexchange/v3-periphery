...
describe('SwapRouter', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    const { weth9, poolDeployer, factory, router, tokens, nft } = await completeFixture(wallets, provider)
  ...
  beforeEach('load fixture', async () => {
    ;({ router, weth9, poolDeployer, factory, tokens, nft } = await loadFixture(swapRouterFixture))
      ...
      describe('multi-pool', () => {
        ...
        it('events', async () => {
          await expect(
            ...
            .withArgs(
              router.address,
              computePoolAddress(poolDeployer.address, [tokens[1].address, tokens[2].address], FeeAmount.MEDIUM),
              3
            )
      ...
      describe('multi-pool', () => {
        ...
        it('events', async () => {
          await expect(
            ...
            .to.emit(tokens[2], 'Transfer')
            .withArgs(
              computePoolAddress(poolDeployer.address, [tokens[2].address, tokens[1].address], FeeAmount.MEDIUM),
              trader.address,
              1
            )
            .to.emit(tokens[1], 'Transfer')
            .withArgs(
              computePoolAddress(poolDeployer.address, [tokens[1].address, tokens[0].address], FeeAmount.MEDIUM),
              computePoolAddress(poolDeployer.address, [tokens[2].address, tokens[1].address], FeeAmount.MEDIUM),
              3
            )
            .to.emit(tokens[0], 'Transfer')

...
import { MockTimeNonfungiblePositionManager, TestERC20Permit, TickLensTest } from '../typechain'
...
describe('TickLens', () => {
  ...
  const nftFixture: Fixture<{
    ...
    const { poolDeployer, factory, tokens, nft } = await completeFixture(wallets, provider)
  ...
  beforeEach('load fixture', async () => {
    ;({ poolDeployer, factory, tokens, nft } = await loadFixture(nftFixture))
  })
  ...
  describe('#getPopulatedTicksInWord', () => {
    ...
    beforeEach(async () => {
      ...
      poolAddress = computePoolAddress(poolDeployer.address, [tokens[0].address, tokens[1].address], FeeAmount.MEDIUM)
    })

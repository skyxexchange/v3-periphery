import { abi as ISkyXV3PoolABI } from '@skyxexchange/v3-core/artifacts/contracts/interfaces/ISkyXV3Pool.sol/ISkyXV3Pool.json'
...
describe('NonfungiblePositionManager', () => {
  ...
  const nftFixture: Fixture<{
    ...
    const { weth9, poolDeployer, factory, tokens, nft, router } = await completeFixture(wallets, provider)
  ...
  let factory: ISkyXV3Factory
  let nft: MockTimeNonfungiblePositionManager
  let tokens: [TestERC20Permit, TestERC20Permit, TestERC20Permit]
  let weth9: IWETH9
  ...
  beforeEach('load fixture', async () => {
    ;({ nft, poolDeployer, factory, tokens, weth9, router } = await loadFixture(nftFixture))
  })
  ...
  describe('#createAndInitializePoolIfNecessary', () => {
    it('creates the pool at the expected address', async () => {
      ...
      console.log("realAddress", await factory.getPool(tokens[0].address, tokens[1].address, FeeAmount.MEDIUM))
    ...
    it('works if pool is created and initialized', async () => {
      ...
      await factory.createPool(tokens[0].address, tokens[1].address, FeeAmount.MEDIUM)
      const pool = new ethers.Contract(expectedAddress, ISkyXV3PoolABI, wallet)
  ...
  describe('#mint', () => {
    ...
    it('creates a token', async () => {
      ...
      await tokens[0].approve(nft.address, 15)
      await tokens[1].approve(nft.address, 15)
      await nft.mint({
        token0: tokens[0].address,
        token1: tokens[1].address,
        fee: FeeAmount.MEDIUM,
  ...
  describe('#collect', () => {
    ...
    it('transfers tokens owed from burn', async () => {
      ...
      const poolAddress = computePoolAddress(poolDeployer.address, [tokens[0].address, tokens[1].address], FeeAmount.MEDIUM)
      await expect(
        nft.connect(other).collect({
          tokenId,
          recipient: wallet.address,
  ...
  describe('multicall exit', () => {
    ...
    it('executes all the actions', async () => {
      const pool = poolAtAddress(
        computePoolAddress(poolDeployer.address, [tokens[0].address, tokens[1].address], FeeAmount.MEDIUM),
        wallet
      )
      await expect(
        exit({
